/*
 * copied from https://github.com/wangzhengbo/JAutoItX/blob/master/src/cn/com/jautoitx/Win32.java
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package net.vpc.common.deskauto.impl.win;
...
// Do nothing
    ...
    public static interface EXT_User32 extends StdCallLibrary {

        final EXT_User32 INSTANCE = (EXT_User32) Native.loadLibrary("user32", EXT_User32.class);
//        boolean EnumWindows (WndEnumProc wndenumproc, int lParam);
//        boolean IsWindowVisible(int hWnd);
//        int GetWindowRect(int hWnd, RECT r);
//        void GetWindowTextA(int hWnd, byte[] buffer, int buflen);

        int GetTopWindow(long hWnd);

        int GetLastActivePopup(long hWnd);

        int GetAncestor(long hWnd, int gaFlags);

        int GetTitleBarInfo(long hWnd, TITLEBARINFO value);
...
//        final int GW_HWNDNEXT = 2;
        ...
        boolean SetWindowPlacement(HWND hWnd, WINDOWPLACEMENT placement);

        boolean ClientToScreen(HWND hWnd, WinDef.POINT point);
    }

    public interface EXT_psapi extends StdCallLibrary {

        EXT_psapi INSTANCE = (EXT_psapi) Native.loadLibrary("psapi", EXT_psapi.class);
    ...
    /**
     *
     * typedef struct tagTITLEBARINFO { DWORD cbSize; RECT rcTitleBar; DWORD
     * rgstate[CCHILDREN_TITLEBAR+1]; } TITLEBARINFO, *PTITLEBARINFO,
     * *LPTITLEBARINFO;
     *
     */
    public static class TITLEBARINFO extends Structure {
        ...
        public int[] rgstate = new int[5 + 1];
    ...
    public static class WINDOWPLACEMENT extends Structure {
        ...
        int flags;
        int showCmd;
        WinDef.POINT ptMinPosition=new WinDef.POINT();
        WinDef.POINT ptMaxPosition=new WinDef.POINT();
        WinDef.POINT rcNormalPosition=new WinDef.POINT();
...
* @param controlHwnd A handle to the control.
     ...
     * the control. If the function fails, the return value is -1. An invalid
     * value for the hwndCtl parameter, for example, will cause the function to
     * fail.
     */
...
* @param hWnd Handle to the window.
    ...
    public static boolean isHWnd(HWND hWnd) {
        ...
        final long lookfor = Pointer.nativeValue(hWnd.getPointer());
        ...
        User32.INSTANCE.EnumWindows(
                new WinUser.WNDENUMPROC() {
                    ...
                    public boolean callback(WinDef.HWND hWnd, Pointer data) {
                        if (lookfor == Pointer.nativeValue(hWnd.getPointer())) {
...
* @param on Turn on capslock if true, otherwise turn off capslock.
    ...
    public static void setCapslockState(boolean on) {
        if (MSWinLib.isCapslockOn() ^ on) {
            // TODO
//            boolean restore = Opt.setSendCapslockMode(false);
//            MSInput.send("{CAPSLOCK}");
//            if (restore) {
...
//            }
    ...
    public static interface GDI32Ext extends GDI32 {
         ...
         *         
* @param fnObject The type of stock object. This parameter can be one
...
* @param hWnd A handle to the window to be enabled or disabled.
         * @param enable Indicates whether to enable or disable the window. If
         * this parameter is true, the window is enabled. If the parameter is
         * false, the window is disabled.
         * @return If the window was previously disabled, the return value is
         * true. If the window was not previously disabled, the return value is
         * false.
         */
...
* @param hwndCtl A handle to the control.
         ...
         * of the control. If the function fails, the return value is zero. An
         * invalid value for the hwndCtl parameter, for example, will cause the
         * function to fail. To get extended error information, call
         * GetLastError.
         * @see <a href=
...
* @param key virtual key. If the desired virtual key is a letter or
         * digit (A through Z, a through z, or 0 through 9), nVirtKey must be
         * set to the ASCII value of that character. For other keys, it must be
         * a virtual-key code. If a non-English keyboard layout is used, virtual
         * keys with values in the range ASCII A through Z and 0 through 9 are
         * used to specify most of the character keys. For example, for the
         * German keyboard layout, the virtual key of value ASCII O (0x4F)
...
* @param hWnd A handle to the owner window of the message box to be
         ...
         * more than one line, you can separate the lines using a carriage
         ...
         * ESC has no effect. If the function fails, the return value is zero.
...
* @param hWnd A handle to the window whose window procedure will
         ...
         * system, including disabled or invisible unowned windows, overlapped
         * windows, and pop-up windows; but the message is not sent to child
         * windows.
         *         
