...
public class BitSet2 implements Cloneable, java.io.Serializable {
     ...
     * @since 1.7
     */
    public static BitSet2 valueOf(long[] longs) {
        int n;
        for (n = longs.length; n > 0 && longs[n - 1] == 0; n--)
            ;
        return new BitSet2(Arrays.copyOf(longs, n));
    }
     ...
     * @since 1.7
     */
    public static BitSet2 valueOf(LongBuffer lb) {
        lb = lb.slice();
        int n;
    ...
    public static BitSet2 valueOf(byte[] bytes) {
        return BitSet2.valueOf(ByteBuffer.wrap(bytes));
    }
     ...
     * @since 1.7
     */
    public static BitSet2 valueOf(ByteBuffer bb) {
    ...
    public void set(int bitIndex, BitSet2 value, int from, int len) {
        if (bitIndex < 0)
            throw new IndexOutOfBoundsException("bitIndex < 0: " + bitIndex);
        if (from < 0)
            throw new IndexOutOfBoundsException("from < 0: " + from);
        if (bitIndex + len >= size())
            throw new IndexOutOfBoundsException("invalid len : " + len);
        //should be optimized!!!
        for (int i = 0; i < len; i++) {
            int k = bitIndex + i;
            if (value.get(i + from)) {
                int wordIndex = wordIndex(k);
                expandTo(wordIndex);
                words[wordIndex] |= (1L << k); // Restores invariants
            } else {
                int wordIndex = wordIndex(k);
                if (wordIndex >= wordsInUse)
                    return;
                words[wordIndex] &= ~(1L << k);
            }
        }
        recalculateWordsInUse();
        checkInvariants();
