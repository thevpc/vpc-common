...
public class LongArrayList {
    ...
    public LongArrayList(long[] values, int offset, int size) {
    ...
    public LongArrayList(long[] values, int size) {
...
//        modCount++;
    ...
    public LongArrayList subList(int offset, int count) {
        if (offset < 0) {
            count += offset;
            offset = 0;
        }
        ...
        return new LongArrayList(this.values, offset, count);
    }
    ...
    public LongArrayList copy() {
        return new LongArrayList(Arrays.copyOf(values, values.length), size);
    }

    public long[] toArray() {
        return Utils.copyOf(values, size);
    }
    ...
    public DoubleArrayList toDoubleArrayList() {
        ...
        return new DoubleArrayList(r, r.length);
    }
    ...
    public IntArrayList toIntCastArrayList() {
        ...
        return new IntArrayList(r, r.length);
    }
...
//        System.out.println(">> "+oldCapacity+" -> "+newCapacity);
    ...
    public void ensureSize(int size) {
        grow(size);
        if (size > values.length) {
            values = Utils.copyOf(values, size);
        }
    }
}
