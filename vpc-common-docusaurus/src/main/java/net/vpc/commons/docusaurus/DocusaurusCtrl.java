 ...
 */
package net.vpc.commons.docusaurus;

import net.vpc.commons.filetemplate.*;
...
import net.vpc.commons.md.docusaurus.DocusaurusFolder;
...
public class DocusaurusCtrl {
    ...
    public DocusaurusCtrl setUpdateSidebarMenu(boolean updateSidebarMenu) {
        this.updateSidebarMenu = updateSidebarMenu;
        return this;
    }
...
//            Files.walk(base).filter(x->Files.isDirectory(base))
        ...
        if (isBuildWebSite()) {
            runCommand(base, getEffectiveNpmCommandPath(), "run-script", "build");
        ...
        if (isStartWebSite()) {
            runCommand(base, getEffectiveNpmCommandPath(), "start");
        }
    }
    ...
    public String getEffectiveNutsCommandPath() {
        String nuts = getNutsCommandPath();
        if (nuts == null || nuts.trim().isEmpty()) {
            return "nuts";
        }
        return nuts;
    }
    ...
    public String getNutsCommandPath() {
        return nutsCommandPath;
    }

    public DocusaurusCtrl setNutsCommandPath(String nutsCommandPath) {
        this.nutsCommandPath = nutsCommandPath;
        return this;
    }

    private void runCommand(Path workFolder, String... cmd) {
        ProcessBuilder pb = new ProcessBuilder(cmd);
        pb.redirectErrorStream(true);
        pb.inheritIO();
        pb.directory(workFolder.toFile());
        int r;
        try {
            r = pb.start().waitFor();
        ...
        if (r != 0) {
            throw new UncheckedIOException("'" + String.join(" ", cmd) + "' failed", new IOException("Process exited with error #" + r));
        }
    }
    ...
    private class FolderConfigProcessor implements TemplateProcessor {
        ...
        public void processPath(Path source, String mimeType, FileTemplater context) {
            ...
            if (
                ...
                cmd.add(nutsCommandPath);
//                cmd.add("--bot");
                cmd.add(autoInstallNutsPackages ? "-y" : "--error");
                ...
                runCommand(Paths.get(target), cmd.toArray(new String[0]));
            }
