 ...
 */
public class DateXmlSerializationDelegate implements XmlSerializationDelegate {
    public void store(XmlSerializer xmlSerializer, Document doc, Element element, Object value) {
        java.util.Date c = (Date) value;
        element.setAttribute("value", String.valueOf(c.getTime()));
    }

    public Object create(XmlSerializer xmlSerializer, Element element, Class clazz) throws XmlSerializationException {
        try {
            return clazz.getConstructor(Long.TYPE).newInstance(Long.valueOf(element.getAttribute("value")));
        } catch (Throwable e) {
