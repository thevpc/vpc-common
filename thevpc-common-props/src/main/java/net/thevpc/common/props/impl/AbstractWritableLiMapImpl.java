...
public abstract class AbstractWritableLiMapImpl<K, V> extends AbstractProperty implements WritableLiMap<K, V> {
    ...
    public AbstractWritableLiMapImpl(String name, PropertyType keyType, PropertyType valueType, Function<V, K> valueToKey) {
...
//            }
    ...
    private class KeySetListImpl extends AbstractWritableListImpl<K> {
        ...
        protected int indexOfImpl(K v) {
            int index = 0;
            for (MapEntry<K, V> e : AbstractWritableLiMapImpl.this) {
        ...
        protected K removeImpl(int index) {
            ...
            for (MapEntry<K, V> e : AbstractWritableLiMapImpl.this) {
                if (index0 == index) {
                    AbstractWritableLiMapImpl.this.remove(e.getKey());
                    return e.getKey();
                }
                index0++;
            }
        ...
        protected int sizeImpl() {
            return AbstractWritableLiMapImpl.this.size();
        }
    }
    ...
    private class EntrySetListImpl extends AbstractWritableListImpl<MapEntry<K, V>> {
        ...
        public Iterator<MapEntry<K, V>> iterator() {
            ...
            return new Iterator<MapEntry<K, V>>() {
                ...
                @Override
                public void remove() {
                    AbstractWritableLiMapImpl.this.remove(curr.getKey());
                }
            };
        }
    }
    ...
    private class ValueListListImpl extends AbstractWritableListImpl<V> {
        ...
        protected V removeImpl(int index) {
            ...
            for (MapEntry<K, V> e : AbstractWritableLiMapImpl.this) {
                if (index0 == index) {
                    V v = AbstractWritableLiMapImpl.this.remove(e.getKey());
                    return v;
                }
                index0++;
            }
        ...
        protected V getImpl(int index) {
            ...
            for (MapEntry<K, V> e : AbstractWritableLiMapImpl.this) {
                if (index0 == index) {
                    return AbstractWritableLiMapImpl.this.get(e.getKey());
                }
                index0++;
            }
            return null;
