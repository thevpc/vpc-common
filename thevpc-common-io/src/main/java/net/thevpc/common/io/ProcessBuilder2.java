    ...
    public ProcessBuilder2 waitFor() throws IOException {
        ...
        if (!baseIO) {
            ...
            if (out != null) {
                ...
                pipes.add(IOUtils.pipe("pipe-out-proc-" + proc.toString(), procInput, out));
            }
            if (err != null) {
                procError = new NonBlockingInputStreamAdapter("pipe-err-proc-" + proc.toString(), proc.getErrorStream());
                if (base.redirectErrorStream()) {
                    pipes.add(IOUtils.pipe("pipe-err-proc-" + proc.toString(), procError, out));
                } else {
                    pipes.add(IOUtils.pipe("pipe-err-proc-" + proc.toString(), procError, err));
                }
            }
            if (in != null) {
                termIn = new NonBlockingInputStreamAdapter("pipe-in-proc-" + proc.toString(), in);
                pipes.add(IOUtils.pipe("pipe-in-proc-" + proc.toString(), termIn, proc.getOutputStream()));
            }
            while (proc.isAlive()) {
                if (termIn != null) {
