...
public abstract class AbstractWritablePMapImpl<K, V> extends AbstractProperty implements WritablePMap<K, V> {
    private PMap<K, V> ro;
    private WritablePList<K> keySet;
    private WritablePList<PMapEntry<K, V>> entrySet;
    private WritablePList<V> valueList;

    public AbstractWritablePMapImpl(String name, PropertyType keyType, PropertyType valueType) {
...
//            }
    ...
    private class KeySetListImpl extends AbstractWritablePListImpl<K> {
        ...
        protected int indexOfImpl(K v) {
            int index = 0;
            for (PMapEntry<K, V> e : AbstractWritablePMapImpl.this) {
        ...
        protected K removeImpl(int index) {
            ...
            for (PMapEntry<K, V> e : AbstractWritablePMapImpl.this) {
                if (index0 == index) {
                    AbstractWritablePMapImpl.this.remove(e.getKey());
                    return e.getKey();
                }
                index0++;
            }
        ...
        protected int sizeImpl() {
            return AbstractWritablePMapImpl.this.size();
        }
    }
    ...
    private class EntrySetListImpl extends AbstractWritablePListImpl<PMapEntry<K, V>> {
        ...
        public Iterator<PMapEntry<K, V>> iterator() {
            ...
            return new Iterator<PMapEntry<K, V>>() {
                ...
                @Override
                public void remove() {
                    AbstractWritablePMapImpl.this.remove(curr.getKey());
                }
            };
        }
    }
    ...
    private class ValueListListImpl extends AbstractWritablePListImpl<V> {
        ...
        protected V removeImpl(int index) {
            ...
            for (PMapEntry<K, V> e : AbstractWritablePMapImpl.this) {
                if (index0 == index) {
                    V v = AbstractWritablePMapImpl.this.remove(e.getKey());
                    return v;
                }
                index0++;
            }
        ...
        protected V getImpl(int index) {
            ...
            for (PMapEntry<K, V> e : AbstractWritablePMapImpl.this) {
                if (index0 == index) {
                    return AbstractWritablePMapImpl.this.get(e.getKey());
                }
                index0++;
            }
            return null;
